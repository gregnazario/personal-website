...
async function optimizeHeadshot() {
	...
	const { size: originalSize } = await sharp(input).metadata().then(() => 
		import("node:fs").then(fs => fs.statSync(input))
	);

	console.log(`📸 Optimizing headshot.png (original: ${(originalSize / 1024 / 1024).toFixed(2)}MB)`);
	...
	console.log(`  ✓ headshot-optimized.png: ${(optimizedPng.length / 1024).toFixed(1)}KB`);
